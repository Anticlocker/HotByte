...
const SearchBar: React.FC<SearchBarProps> = ({
  ...
  return (
    <div
      className={`flex items-center h-9 rounded-full border bg-white dark:bg-zinc-900/80 transition-all duration-300 relative shadow-sm max-w-full ${
        isFocused
    ...
    >
      <div className="flex items-center justify-center w-8 h-8 shrink-0 text-gray-400 dark:text-gray-550 pl-1">
        <Search size={13} className={isFocused ? "text-orange-500" : ""} />
      </div>
      ...
      <input
        ...
        className="w-full bg-transparent border-0 outline-none text-[11px] font-semibold text-gray-800 dark:text-gray-200 placeholder:text-gray-400 dark:placeholder:text-gray-500 py-1 pr-6 pl-0"
      />
      ...
      {hotelType === "both" && (
        <div className="flex items-center shrink-0 pr-1 h-full">
          ...
          <button
            ...
            className={`w-7 h-7 rounded-full flex items-center justify-center transition-all duration-200 select-none active:scale-90 ${
              isVegOnly
