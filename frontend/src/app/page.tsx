...
export default function Home() {
  ...
  const handleSubscriptionPurchase = async (rawPlanName: string) => {
    ...
    // Redirect to the redesigned SaaS multi-step onboarding wizard
    router.push(`/onboarding?plan=${planName}&billing=${billingCycle}`);
  };
  ...
  const pricingPlans = [
    {
      ...
      yearlyPrice: 1,
      period: "for 14 days",
      badge: "Pro Features Trial",
      badgeCls: "bg-orange-500/10 text-orange-400 border-orange-500/20",
      features: ["Full platform access for 14 days", "Includes all Pro features during trial", "Razorpay active payment gateway", "Live kitchen display system (KDS)", "Dynamic QR codes per table", "Daily PDF sales reports"],
      cta: "Start 14-Day Trial for ₹1",
      ctaCls: "bg-gradient-to-r from-orange-500 to-amber-600 text-white shadow-lg shadow-orange-500/20 hover:scale-[1.02] transition-transform"
    },
    {
      name: "Basic Plan",
      monthlyPrice: 999,
